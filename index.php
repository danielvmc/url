...
$app->post('/admin/login', function () use ($app) {
    ...
    if ($config['user'] == $user && $config['password'] == $pass) {
        $_SESSION['user'] = $user;
        $_SESSION['pass'] = $pass;
        $app->redirect($config['url'] . '/admin/create_link');
    } else {
        $app->redirect($config['url'] . '/index.php/admin');
    }
});
...
// Admin Page
$app->get('/admin/create_link', $authenticate($app), function () use ($app) {
    $twig_vars = $app->config('twigVars');
...
// Admin Page Id
$app->get('/admin/link/:slug', function ($slug) use ($app) {
    $twig_vars = $app->config('twigVars');
